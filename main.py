...
def download_data():
    ...
    for date in tqdm(df["Date"].unique(), desc="Saving data"):
        # Do not save data of the current day, as it is not complete yet
        if date == pd.Timestamp.now().date():
            tqdm.write(f"Skipping {date} as it is the current day")
            continue
...
def main(
    ...
    for file in sorted(files):
        if timeframe_start and timeframe_start.date() > pd.Timestamp(file.stem).date():
            continue
        if timeframe_end and timeframe_end.date() < pd.Timestamp(file.stem).date():
            continue
    ...
    if plot_phases:
        ...
        y_columns = ["Total Active energy Wh", "Total Returned energy Wh"]
    ts.plot(y=y_columns,
...
if __name__ == "__main__":
    ...
    main(args.download, args.sample_rate, args.plot_phases, args.timeframe_start, args.timeframe_end)
